<template>
	<nav class="panel has-shadow">
		<div class="panel-block is-size-4">
			<fa icon="list" class="is-size-6" />&nbsp;<span class="is-size-5 is-bold">Rejected Claims</span>
		</div>
		        ...
		        <template slot="empty">
	                <section class="section">
	                    <div class="content has-text-grey has-text-centered">
	                        ...
	                        <p>You have no rejected bounty claims</p>
	                    </div>
	                </section>
	            </template>
	...
	export default {
		...
		methods: {
			...
			fetchClaims: async function(bountyId) {
				var { data } = await axios.get(`/api/bounty/claims/rejected`)
				if (data.status !== 'error') {
					this.claims = data.data.claims
				} else {
					this.$toast.open({ text: 'Error loading rejected claim data', type: 'error' })
				}
			},
